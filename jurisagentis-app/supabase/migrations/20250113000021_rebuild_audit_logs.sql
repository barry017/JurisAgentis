-- Align legacy audit log structure with enhanced document management schema
-- Safe to run multiple times; only transforms when legacy columns detected

BEGIN;

-- Rename legacy table (identified by presence of old columns)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'audit_event_type') THEN
    CREATE TYPE audit_event_type AS ENUM (
      'auth_login','auth_logout','auth_failed_login','auth_mfa_setup','auth_mfa_verify',
      'auth_password_change','user_role_change','user_status_change','allowlist_add',
      'allowlist_remove','temp_access_grant','temp_access_revoke','session_terminate',
      'data_access','unauthorized_access_attempt','create','read','update','delete','archive',
      'restore','generate_from_template','create_version','download','print','share',
      'grant_access','revoke_access','modify_permissions','login','logout','failed_login',
      'add_comment','resolve_comment','mention_user','react_to_comment','create_signature_request',
      'sign_document','decline_signature','cancel_signature_request','send_reminder',
      'create_template','update_template','use_template','archive_template','backup_created',
      'system_maintenance','security_scan','data_export','data_import','configuration_change',
      'start_workflow','complete_workflow','fail_workflow','pause_workflow','resume_workflow'
    );
  ELSE
    PERFORM 1;
    -- Ensure new values exist
    FOREACH val IN ARRAY ARRAY['create','read','update','delete','archive','restore',
      'generate_from_template','create_version','download','print','share','grant_access',
      'revoke_access','modify_permissions','login','logout','failed_login','add_comment',
      'resolve_comment','mention_user','react_to_comment','create_signature_request',
      'sign_document','decline_signature','cancel_signature_request','send_reminder',
      'create_template','update_template','use_template','archive_template','backup_created',
      'system_maintenance','security_scan','data_export','data_import','configuration_change',
      'start_workflow','complete_workflow','fail_workflow','pause_workflow','resume_workflow']
    LOOP
      BEGIN
        EXECUTE format('ALTER TYPE audit_event_type ADD VALUE %L', val);
...
BEGIN
  ...
  ELSE
    FOREACH val IN ARRAY ARRAY['low','medium','high'] LOOP
      BEGIN
        EXECUTE format('ALTER TYPE audit_severity ADD VALUE %L', val);
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'audit_entity_type') THEN
    CREATE TYPE audit_entity_type AS ENUM (
      'authentication','user_management','user_access','user_profile','system','document',
      'document_version','template','signature_request','comment','user','matter','client',
      'share_link','configuration','workflow','workflow_execution','workflow_step','calendar_event',
      'calendar_deadline','integration','search','retention_policy'
    );
  END IF;
END;
$$ LANGUAGE plpgsql;

-- Replace legacy structures with enhanced versions
...
CREATE INDEX IF NOT EXISTS idx_audit_logs_action_timestamp ON audit_logs(action_type, timestamp);
CREATE INDEX IF NOT EXISTS idx_audit_log_search_vector ON audit_log_search_index USING GIN(search_vector);
...
DROP POLICY IF EXISTS audit_log_attachments_access_policy ON audit_log_attachments;
...
DROP POLICY IF EXISTS audit_log_search_index_access_policy ON audit_log_search_index;
...
DROP TRIGGER IF EXISTS audit_log_search_vector_trigger ON audit_logs;
...
DROP TABLE IF EXISTS audit_logs_legacy CASCADE;
